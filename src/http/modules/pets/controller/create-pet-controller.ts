...
export async function createPetController(req: FastifyRequest, res: FastifyReply){
  const { nome, sobre, idade, porte, nivelEnergia, nivelIndependencia, ambiente, foto, requisito } = createPetBodySchema.parse(req.body);
  const ongId = req.user.sub as string;

  try {
    const createPetService = makecreatePetService();
    const newPet = await createPetService.execute({
      nome, sobre, idade, porte, nivelEnergia, nivelIndependencia, ambiente, foto, requisito, ongId,
    });
